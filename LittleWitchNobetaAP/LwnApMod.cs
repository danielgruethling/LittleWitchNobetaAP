...
{
    public bool ShowApUI { get; set; } = false;
    public static ArchipelagoClient? ArchipelagoClient {get; private set;}
    
    public const string ModDisplayInfo = $"{MyPluginInfo.PluginName} v{MyPluginInfo.PluginVersion}";
    ...
    private void Update ()
    {
        ...
        else // not in menu so can be death linked
        {
            // try to kill player
    ...
    private static void ArchipelagoConfigEditorWindow (int windowID)
    {
        ...
        var statusMessage =
            ArchipelagoClient.Authenticated ? " Status: Connected" : " Status: Disconnected";
